# Boas vindas ao repositório do futebol_clube
...
Nesse projeto, construi um back-end dockerizado utilizando modelagem de dados através do `Sequelize`. O desenvolvimento respeitou regras de negócio providas no projeto e a API construída com as bibliotecas `Node.js`, e `Express.js` foi capaz de ser consumida por um front-end já provido.
...
O banco de dados `MySQL` em um relacionamento entre as tabelas teams e matches para fazer as atualizações das partidas.
...
```bash
  git clone git@github.com:DenisDaros/futebol_clube_repositorio.git
```
